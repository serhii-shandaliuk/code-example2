 ...
 */
class AddTeacherForm extends FormBase {
 ...
 */
  ...
  public function buildForm(array $form, FormStateInterface $form_state, string $uuid = NULL): array {
    ...
    $form['email'] = [
      '#type' => 'textfield',
      '#title' => $this->t("Teacher's or author's email or name"),
      '#description' => $this->t('If the teacher or author has not created an account, they will receive an email with instructions.'),
      '#required' => TRUE,
  ...
  public function validateForm(array &$form, FormStateInterface $form_state): void {
    ...
    elseif (empty($role)) {
      $form_state->setError($form['email'], $this->t('Role field is required'));
    }
    else {
      ...
      if (isset($te['#name'])) {
        ...
        if ($te['#name'] === 'delete') {
          if (!empty($user)) {
            ...
            $school_entity = $school_storage->loadByProperties(['field_author' => $this->currentUserEntity->id()]);
            $teachers = [];
            if ($school_entity) {
            ...
            if (!in_array($user->id(), $existing)) {
              $form_state->setError($form['email'], $this->t('User you have tried to delete from your teaches does not your teacher or author any more.'));
            }
          }
        }
        ...
        elseif (!empty($user)) {
          ...
          if ($this->currentUser->id() === $user->id()) {
            $form_state->setError($form['email'], $this->t('You can not add yourself as a teacher or author.'));
          }
        }
        if (empty($user)) {
    ...
    if (isset($te['#name'])) {
      ...
      if ($te['#name'] === 'add') {
        if (!in_array($user_id, $existing)) {
          $teachers += ['target_id' => $user_id];
        }
        $plan = $this->currentUserEntity->get('field_subscription_type')->value;
        $this->schoolManager->giveTeacherAccess($user_id, $role . ';' . $plan, NULL, '+1 month');
        $action = 'added';
      }
      else {
        ...
        $this->schoolManager->revokeTeacherAccess($user_id, $role, FALSE);
      }
      ...
      $this->messenger()->addMessage($this->t('You successfully @action @role.', ['@action' => $this->t($action), '@role' => $this->t($role)]));
      $message = [
        '#type' => 'status_messages',
        '#weight' => -10,
      ];
